...
public class NetworkService {
    ...
    public IEnumerator GetLinksJSON(Action<string> callback)
    {
        return CallAPI(jsonApi, null, callback);
    }

    public IEnumerator GetImagesJSON(Action<string> callback)
    {
        return CallAPI(jsonApi, null, callback);
    }
	...
	public IEnumerator DownloadImage(Action<Texture2D> callback, String weburl) {
		WWW www = new WWW(weburl);
		yield return www;
		callback(www.texture);
	}

    public IEnumerator DownloadPage(Action<string> callback, String weburl)
    {
        WWW www = new WWW(weburl);
        yield return www;
        callback(www.text);
